export const jobs = [
  {
    id: 1,
    title: "Stage PFE",
    company: "Villa52",
    time: "Mar 2022 - Apr 2022",
    points: [
      "Build a website from scratch using WordPress and connect it with an e-commerce platform ( WooCommerce ).",
      "migrate the source code of another website into WordPress.",
      "hosting, customization, and theming those two websites.",
    ],
  },
  {
    id: 2,
    title: "Frontend Web Developer Intern",
    company: "WimTech",
    time: "Oct 2023 - Jan 2024",
    points: [
      "I'm working at Wimtech as a front-end developer intern. I use many amazing technologies on a daily basis, such as React, NextJS, Tailwind CSS, etc.",
      "I contributed to an existing project by adding new features and fixing some issues.",
      "Skills: REST APIs · React.js · Next.js · Tailwind CSS · Angular · Git ..."
    ],
  },
  {
    id: 3,
    title: "Full-stack Developer",
    company: "WimTech",
    time: "",
    points: [
      "As a full-stack developer at Wimtech, I successfully addressed diverse challenges by leveraging a range of technologies, contributing to the development of robust solutions.",
      "Throughout my role, I adeptly navigated between front-end and back-end development tasks, collaborating on various projects. This involved implementing new features, resolving issues, and ensuring the seamless integration of multiple technologies.",
      "Skills: Angular · Bootstrap · React · REST API · Spring Boot · Git/GitLab ..."
    ],
  },
  {
    id: 4,
    title: "",
    company: "New place!",
    time: "",
    points: [],
  },
  {
    id: 5,
    title: "",
    company: "New place!",
    time: "",
    points: [],
  },
  {
    id: 6,
    title: "",
    company: "New place!",
    time: "",
    points: [],
  },
];

export const projects = [
  {
    id: 1,
    title: "Random Quote Generator",
    description:
      "this is a ReactJS API data fetching project. We have React on the front end and we'll make get requests to Advice Slip JSON API.",
    tech: ["React.js", "API", "Bootstrap"],
    github: "https://github.com/Mr-Badr/Random-Quote-Generator",
    live: "https://great-random-quotes.netlify.app",
  },
  {
    id: 2,
    title: "Weather application",
    description:
      "React JS weather app using the OpenWeatherMap API. We will be using Axios to connect with our API.",
    tech: ["React.js", "Axios", "API"],
    github: "https://github.com/Mr-Badr/weather-app-react",
    live: "https://weather-web-appp.netlify.app",
  },
  {
    id: 3,
    title: "To Do List",
    description:
      "JavaScript Todo List helps you create a list of things you want to do throughout the day",
    tech: ["HTML", "CSS", "JavaScript"],
    github: "https://github.com/Mr-Badr/vanilla-todo-list",
    live: "https://mr-badr.github.io/vanilla-todo-list/",
  },
  {
    id: 4,
    title: "Google Maps Clone",
    description: "A simple Google Maps Clone built with Vanilla JavaScript.",
    tech: ["HTML", "CSS", "JavaScript"],
    github: "https://github.com/Mr-Badr/Google-Maps-Clone",
    live: "https://mr-badr.github.io/Google-Maps-Clone/",
  },
  {
    id: 5,
    title: "Simple Responsive Website",
    description: "This is a Responsive Website built only with HTML and CSS.",
    tech: ["HTML", "CSS", "Media Queries"],
    github: "https://github.com/Mr-Badr/Travel-website",
    live: "https://mr-badr.github.io/Travel-website/",
  },
  {
    id: 6,
    title: "Student Grading System",
    description:
      "In this Grading System In Java has Graphical User Interface (GUI) Design, and this system is a user friendly that anyone can manage in easy way.",
    tech: ["Java"],
    github: "https://github.com/Mr-Badr/Student-score",
    live: "https://photos.google.com/share/AF1QipNkN2sk9tJ4jrVFlx7vQOrEmYSDFlUss1KSxoIvVcG_ux1PIpUaKeqiNm0DIIC0gQ/photo/AF1QipNgrUpf54esA-wcK4od64J_4BxDhDlKeNogt3Rt?key=bjNvZFNpUlZ3SXZ2MERKNnFQMkwxUDJLbVhFM0t3",
  },
];
